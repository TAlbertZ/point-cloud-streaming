...
class HiddenPointsRemoval():
    ...
    def sphericalFlip(self, center, param):
        ...
        points = self.points - np.repeat(center, n,
                                         axis=0)  # Move C to the origin
        normPoints = np.linalg.norm(
            points, axis=1)  # Normed points, sqrt(x^2 + y^2 + (z-100)^2)
        # print(normPoints)
        # print(max(normPoints))
        R = np.repeat(max(normPoints) * np.power(10.0, param), n,
                      axis=0)  # Radius of Sphere
        # print(R)
        # pdb.set_trace()
        R = np.repeat(1.5, n, axis=0)  # Radius of Sphere

        flippedPointsTemp = 2 * np.multiply(
            np.repeat((R - normPoints).reshape(n, 1), len(points[0]), axis=1),
            points)
        flippedPoints = np.divide(
            flippedPointsTemp,
            np.repeat(normPoints.reshape(n, 1), len(points[0]),
    ...
    def plot(self, visible_hull_points):
        ...
        plt.title(
            'Test Case With A Sphere (Left) and Visible Sphere Viewed From Well Above (Right)'
        )
        ...
        for vertex in visible_hull_points.vertices[:-1]:
            ax.scatter(self.points[vertex, 0],
                       self.points[vertex, 1],
                       self.points[vertex, 2],
                       c='b',
                       marker='o')  # Plot visible points
