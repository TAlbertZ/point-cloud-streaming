...
# BYTE_SIZES = [BYTE_SIZES[0], BYTE_SIZES[1], BYTE_SIZES[2]] # originally, according to the img sent from Yixiang
...
class Buffer():
	...
	def emit_buffer(self):
		...
		for frame_idx in range(self.current_viewing_frame_idx + 1, self.current_viewing_frame_idx + FPS * UPDATE_FREQ + 1):
			...
			z_weights = self.calculate_z(viewing_probability, distances, frame_idx, frame_idx)
	...
	def update_tile_size_in_buffer(self):
		...
		tiles_rate_solution, buffered_tiles_sizes, sum_solution_rate, sum_r0 = self.kkt(np.array(z_weights), predicted_bandwidth_budget * Mbps_TO_Bps, update_start_idx, update_end_idx)
	...
	def kkt(self, z_weights, bandwidth_budget, update_start_idx, update_end_idx):
		...
		# each tile has a 4 tuple location: (frame_idx, x, y, z)
		tiles_rate_solution = np.zeros((num_frames_to_update, NUM_TILES_PER_SIDE_IN_A_FRAME, NUM_TILES_PER_SIDE_IN_A_FRAME, NUM_TILES_PER_SIDE_IN_A_FRAME))
