...
class iLQR_solver():
    ...
    def set_predicted_bw_rtt(self):
        ...
        self.predicted_bw = [np.round(bw, 2)*self.bw_ratio for bw in predicted_bw]
    ...
    def set_initial_action(self, action_size):
        ...
        us_init = np.random.uniform(0.1, 1, (self.n_step, action_size))
    ...
    def translate_to_speed_idx_accu(self):
        # first_speed = self.speeds[0]
        first_speed = np.mean(self.speeds[:5])
        # print("speeds: ", self.speeds)
        # print("<-------------END------------>")
        distance = [np.abs(first_speed-s) for s in SPEED]
        speed_idx = distance.index(min(distance))
    ...
    def translate_to_rate_idx(self):
        first_action = np.mean(self.rates[:1])
